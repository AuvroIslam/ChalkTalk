...
class Canvas(QWidget):
    ...
    def _tick(self) -> None:
        now = time.monotonic()
        for k, (it, start) in enumerate(self.items):
            if start <= now and k not in self.announced:
                self.announced.add(k)
                if it.say:
                    self.item_started.emit(it.say)
        if all(now > s + i.duration + 0.05 for i, s in self.items):
            self.timer.stop()
        self.update()
